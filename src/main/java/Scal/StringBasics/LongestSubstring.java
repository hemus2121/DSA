package Scal.StringBasics;

import java.util.*;
public class LongestSubstring {
    private static int getLength(String A){
        int l=0,r=0;
        int ans = Integer.MIN_VALUE;
        int n = A.length();
        Set<Character> dataset= new HashSet();
        if (n==1) return 1;
        while (r<A.length() ){
            if (dataset.contains(A.charAt(r))){
                dataset.remove(A.charAt(l));
                l++;
            }
            else {
                dataset.add(A.charAt(r));
                r++;
                ans = Math.max(ans,dataset.size());
            }
        }
        return ans;
    }
    public static void main(String[] args) {
        System.out.println(getLength("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"));
        System.out.println(getLength("dadbc"));
        System.out.println(getLength(""));

    }
}
